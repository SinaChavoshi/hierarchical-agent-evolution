"""
Harvests all Generation 7 scorecards (including mutant_3), ranks the population,
...
def harvest():
    os.makedirs("experiments/exp-009-parallel-gen7/scorecards", exist_ok=True)
    token = subprocess.check_output(['gcloud', 'auth', 'application-default', 'print-access-token']).decode().strip()

    # List items in GCS
    list_req = urllib.request.Request(
        'https://storage.googleapis.com/storage/v1/b/gemle-gke-dev-agent-evolution/o?prefix=parallel_runs/generation_7/',
    ...
    for item in items:
        ...
        local_path = os.path.join("experiments/exp-009-parallel-gen7/scorecards", fname)
    ...
    sc_files = sorted(glob.glob("experiments/exp-009-parallel-gen7/scorecards/*.json"))
    data = []
    for f in sc_files:
        with open(f) as fp:
            d = json.load(fp)
    ...
    print("\n" + "="*85)
    print("=== GENERATION 7 TOURNAMENT OFFICIAL FINAL STANDINGS ===")
    print("="*85)
    ...
    for i, d in enumerate(data, 1):
        ...
        files_c = verif.get('files_found_count', 0)
    ...
    champ_path = "experiments/exp-009-parallel-gen7/winning_champion_genome.json"
    with open(champ_path, "w") as f:
        json.dump(champ_genome, f, indent=2)
    print(f"\nSaved Generation 7 Champion Genome to {champ_path} ({champ['company_id']}, Net: {champ.get('overall_score')})")
    ...
    print(f"Saved Generation 7 Top 5 Survivor Genomes to {top_5_path}")
